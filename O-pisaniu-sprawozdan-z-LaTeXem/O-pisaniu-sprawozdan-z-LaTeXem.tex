...
\begin{itemize}
\item[--] W~tekście na~pewno~są obecne błędy, będziemy wdzięczni
  za~przesyłanie ich \tb{kziemianfvt@gmail.com}.
\item[--] Wszelkie propozycje jak go ulepszyć można kierować pod
  powyższy mail. Postaramy~się je rozpatrzyć i~maksymalnie szybko
  wprowadzić.
\item[--] Tekst nie tylko można, ale należy przekazywać dalej, każdemu
  komu może~się on~przydać.
\item[--] Jeśli wprowadzi~się do~niego jakieś poprawki samemu, prosimy
  by dopisać~się do listy autorów, a~jedną wersję przesłać do~nas
  mailowo. W~ten sposób, będziemy mogli zawsze rozpowszechniać
  możliwie najlepszą wersję.
...
\item[--] Jeżeli treść jakiegoś punktu wydaje~się tak oczywista,
  że~nie warto nawet o~tym wspominać, to~zapewne została tu
  umieszczona, bo~pojawiła~się w~jednym, bądź kilku, poprawianych
  przez autorów sprawozdaniach. Banalne błędy wyglądają zwykle
...
\item[--] Staraliśmy~się żeby tekst ten był możliwie poprawny
  ...
  i~standardy. Dlatego staraliśmy~się konsultować te~miejsca gdzie
  nasza wiedza nie jest zbyt głęboka. Mimo tego pewne archaizmy
  i~błędy mogły pozostać, prosimy używać niniejszych porad z~rozwagą.
\item[--] Zdania uczonych~są podzielone w~wielu sprawach, wliczając
  w~to standardy pracy i~konwencji. Stąd nawet jeśli jakiś punkt w~tym
  tekście jest napisany zupełnie poprawne, w~zgodzie z~tym jak wiele
  osób robi, ktoś może wymagać zupełnie innego podejścia do~sprawy.
...
\item[--] Wszystkie wielkości fizyczne muszą podawać jawnie jednostki
  w~których~są wyrażone\footnote{Chyba, że~jest to wielkość
    bezwymiarowa, jak ilość przebadanych kondensatorów.}!!!
...
\item ,,Błąd pomiaru'', ,,niepewność pomiarowa'', ,,dokładność
  ...
  nazwy na jedno i~to samo\footnote{Z~pewnymi wyjątkami o~których
    będzie później.}! Ponieważ jest strasznie mylące nazywać tą~samą
  ...
  standardowe''. \tothink{Mam poważne wątpliwości co do tego czy
    odchylenie standardowe jest takie neutralne -- przedyskutujmy to.}
\item Odchylenie standardowe ma~zawsze ten sam wymiar jak wielkość
...
\item Wedle obecnie przyjętych norm odchylenie standardowo
  ...
  cyfr poczynając od~pierwszej nieznikającej. Przykłady znajdują~się
  w~poniższej tabeli.
  \begin{figure}[h]
    \centering
...
\item Przyjęło~się, że jeśli drugą cyfrą znaczącą odchylenia
  standardowego jest zero to zapisujemy je, nawet jeśli wydaje się
  niepotrzebne. Czyli odchylenie~0.2 należy zapisywać jako~0.20.
  Robiąc inaczej moglibyśmy wywołać błędne wrażenie, że zaokrąglamy do
...
\item Jeżeli zaokrąglamy liczbę kończącą~się cyfrą 5 np.~47.95, to
  ...
  uczonych w~tej sprawie są podzielone, można~się przekonać czytając
  artykuł na~angielskiej
  \href{https://en.wikipedia.org/wiki/Rounding}{Wikipedii
    --~Rounding}. Polecana tu~metod nosi tam nazwę \emph{Round half
    to~even}.
\end{enumerate}
...
\item Niepewności dzielą~się na
  ...
  \end{enumerate}
\item Błędy grube, to wszystkie sytuacja kiedy eksperymentator zrobił
  coś mocno źle, albo aparatura zawiodła. Np.~ktoś zmierzył masę
  próbki, nie zdejmując z~wagi swojego kubka z~herbatą; podczas
  pomiaru doszło do~awarii sieci elektrycznej, w~skutek czego napięcie
  spadło do~$150 \si{.V}$ i~cały pomiar jest bezwartościowy. Każdy
  pomiar obarczony błędem grubym należy usunąć
  z~rozważań\footnote{Chyba, że~ma~się \tb{naprawdę} dobry powód,
    by~go zostawić.}.
\item Błędy systematyczne, to~błędy ,,wbudowane'' w~sam sposób
  przeprowadzania danego eksperymentu, są więc popełniane
  ...
  $\Delta L = 0.1 \times 10^{ -2 } \si{.m}$. 
  
  \toadd{O ile opisany powyżej rodzaj błędów systematycznych jest łatwy do
  zauważenia i oszacowania istnieje inny znacznie groźniejszy rodzaj.
  Może się zdażyć, że efekt fizyczny, na którym opieramy pomiar,
  lub teoria na podstawie której łączymy bezpośrednio zmierzone
  wielkości fizyczne z poszukianą wielkością powoduje np. systematyczne
  zawyżanie wyników} 
  \todel{Błędy systematyczne mogą pochodzić też z~bardziej subtelnych źródeł.
  Przykładowo sam efekt fizyczny na~którym bazuje metoda pomiaru,
  powoduje systematyczne zawyżanie wyników.}

  Należy podkreślić, że~błędy systematyczne \tb{nie} muszą być znane
  eksperymentatorom, co jest dużym problemem w~eksperymentach.
  Przykładowo, w~badanym procesie fizycznym występuje dodatkowe
...
\item Niepewności przypadkowe to te, które wynikają z~istnienia
  słabych, losowych efektów, których \tb{zawsze należy~się
  ...
  \eqref{sec:estymator}.
\item Niepewność eksperymentu to~ostateczna niepewność całego
  ...
  $\Delta_{ \textrm{s} } x$, wówczas niepewność eksperymentu
...
\item Załóżmy, że~nie jesteśmy w~stanie obliczyć niepewności
  przypadkowej (o~tym jak to zrobić będzie mowa dokładniej
  w~podparagrafie \eqref{sec:obliczanie}). Przyczyna może być taka,
  że~np.~mierzy długość małej płytki linijką i~jej najmniejsza
  przedziałka jest na tyle duża, że~wszystkie wyniki są~jednakowe.
  Inny przypadek jest taki, że~z~jakiegoś powodu wykonaliśmy tylko
  jeden pomiar danej wielkości.
  ...
  mieszczącego wszystkie zaobserwowane wyniki, która nie może być
  mniejsza niż błąd systematyczny pomiaru. W~poprzednim przykładzie
  $L$~będzie równe szerokości najmniejszego podziału na~linijce. Jeśli
  wykonaliśmy tylko jeden pomiar za~$L$ przyjmujemy po~prostu błąd
  systematyczny pomiaru. Całemu pomiarowi przypisujemy wtedy
  niepewność
  \begin{equation}
    \label{eq:2}
...
\item Pewne błędy mogą być zarówno systematyczne jak i~grube.
  Na~przykład, jeśli eksperymentator źle skalibrował wagę, wskutek
  czego zaniża ona o~stałą wartość wagę mierzonych próbek.
\end{enumerate}
...
i~94\textcelsius, to~całkiem naturalne jest przyjęcie, że~lepszy
oszacowaniem prawdziwej temperatury niż wybranie jednej z~nich wartości,
będzie wzięcie ichwartości średniej ich~obu: 93.5\textcelsius.
...
pozwala nam wyliczyć to~oszacowanie nazywamy \tb{estymatorem}, zaś~sam
proces szukania takiej wartości \tb{estymacją}. Typowym sposobem
szacowania prawdziwej wielkości na~podstawie $n$~pomiarów jest wzięcie
...
jest najlepsze?'' (czyli który estymator jest najlepszy) okazuje~się
wcale niebanalne. Dlatego dla jednej wielkości mogą istnieć różne
estymatory, zależy która ich własność jest przez nas pożądana, jednak
nie będziemy~się tu w~ten temat zagłębiać.
...
Gaussa\footnote{Nie będziemy tu wchodzić bardziej w~teorię
  prawdopodobieństwa, te rozważania są tylko po~to by uzasadnić to co
  jest dalej.}
\begin{equation}
  \label{eq:4}
...
przypadkowe, więc jest on rozpisywany rozkładem Gaussa i~mamy
...
\item \tb{Odchylenie standardowe średniej pomiarów}
  ...
  \end{equation}
\end{enumerate}
\tb{Ważne.} Nie wdając~się w~subtelne dywagacje, można podać
...
\tb{Ważne 2.}\toadd{ Jak zaznaczyliśmy wyżej, centralne twierdzenie graniczne
działa dla liczby pomiarów dążącej do nieskończoności. Dość powszechnie choć
arbitralnie przyjmowaną wielkością, którą można uznać za nieskończoną jest 30
\footnote{Choć można spotkać osoby uznające, że dobra statystyka pozwalająca
na spokojne stosowanie rozkładu Gaussa zaczyna się od setki pomiarów
-- zalecamy zachowanie czujności.}. Gdy dysponujemy małą liczbą pomiarów warto
z poprawek wynikających z zastosowania rozkładu Studenta zamiast Gaussa
-- popularnych współczynników Studenta-Fishera.
}
...
z~taką niepewnością.
\begin{enumerate}
\item Po~wypisaniu wyniku podajemy w~nawiasie dwie cyfry
  reprezentującą niepewność, następnie zaś~jednostkę danej wielkości
  fizycznej. Podany wyżej przykład zapisujemy więc jako
  \begin{equation}
    \label{eq:8}
    x = 1.335(15) \si{.m},
  \end{equation}
  ...
  Zaleca~się używania tej konwencji.
\item W~tym przypadku używamy \tb{rozszerzonej niepewności
  ...
  należy \tb{zawsze} jawnie stwierdzić, że~używa~się rozszerzonej
  niepewności i~podać użytą do~jej obliczania wartość~$k$. [PE: to w
  końcu trzeba czy nie trzeba?]
  ...
  Te liczy pochodzą z~własności rozkładu Gaussa\footnote{Tutaj parę
    rzeczy opisaliśmy zbyt skrótowo. Może kiedyś napiszemy to lepiej.}
  (patrz:\label{sec:obliczenia}). Prawdopodobieństwo znalezienia w
  przedziale wartości rzeczywistej, dla~dowolnego parametru~$k$, można
  otrzymać obliczając samemu na~podstawie tego rozkładu albo~biorąc
  je~z~odpowiednich
...
\item Jeśli plik \LaTeX a zawiera odniesienia do~wzorów
  ...
  pojawiły~się w~odpowiednich miejscach zamiast np.~,,(??)''. Należy
  sprawdzić, czy~używane środowisko ma~możliwość przypisania
  do~jednego przycisku wykonania serii następujących po~sobie
  kompilacji. Na~przykład w~\TeX Makerze można wybrać
  w~\emph{Preferencje} $\to$~\emph{Konfiguracja \TeX Makera}
...
\item Jeśli otwierasz jakieś otoczenie\footnote{Zwane
  ...
  ,,\texttt{\tbs end\{otoczenie\}}''! Nie mów sobie ,,Na~pewno
  zapamiętam je zamknąć.'', \tb{prawie zawsze} o~tym zapomnisz,
...
\item Polecenia \LaTeX a zaczynają się ukośnikiem wstecznym
  ...
  Jednak już
  \begin{quote}
    \verb+\LaTeX to~potężne narzędzie.+
  \end{quote}
  daje
  \begin{quote}
  ...
  odstęp kończyć \verb+{}+. Przykładowo
  \begin{displayquote}
    \verb+\LaTeX{} to~potężne narzędzie.+
  \end{displayquote}
  wyświetli tekst
...
\item ,,Wszystko napisałem dobrze, a~\LaTeX{} wyrzuca błędy!''. Tego
  ...
  wpisanym wzorze matematycznym. Pozwala to~od~razu znajdować błędy.
  Nie należy~się też poddawać, jeśli przeglądasz linię, gdzie mam być
  błąd po~raz czwarty i~wszystko wygląda na~poprawne. Czasem dopiero
  za~siódmym razem znajdziesz błąd.
  ...
  Makerze komentarz tworzymy za~pomocą ,,Ctrl + T'', a~odkomentujemy
  ,,Ctrl + U''. Staną~się niedługo jednymi z~twoich najlepszych
  przyjaciół przy pracy w~\LaTeX u.
...
\item Oznaczenia zmiennych i parametrów w tekście powinny być pisane
  ...
  wpisać ,,\verb+$z$+''~nie~,,\verb+z+'', tak by~otrzymać~,,$z$''~nie
  zaś~,,z''.
...
\item Komendy ,,\texttt{\tbs left}'' i~,,\texttt{\tbs right}'' dają
  ...
  przykładu:~,,\verb+$( \frac{ G M }{ r } )$+
  vs~\verb+$\left( \frac{ G M }{ r } \right)$+'' daje
  ,,$( \frac{ G M }{ r } )$ vs~$\left( \frac{ G M }{ r } \right)$''.
  Możliwe jest też mieszanie typów nawiasów lewych i~prawych:
...
\item \LaTeX{} sam zarządza odstępami w~trybie matematyczny, co czasem
  ...
  ,,\verb+$a b$+'' daje ,,$a b$'', zaś ,,\verb+$a, b$+''
  ...
  Tym sposobem ,,\verb+$a\, b$+'' daje ,,$a\, b$'',
  zaś~,,\verb+$a\, b\: c\; d$+'' prowadzi do~,,$a\, b\: c\; d$''.
...
i~bardziej rozbudowany. Pakiet zmienia~się na~tyle często, że~wersja
obecna w~danym repozytorium \TeX a może nie działać, tak jak to jest
przedstawione w~dokumentacji pakietu. Powoduje to, że omawianie tutaj
pakietu \texttt{mandi} jest bezcelowe. Zainteresowanych odsyłamy
dokumentacji
na~\href{ftp://ftp.gust.org.pl/TeX/macros/latex/contrib/mandi/mandi.pdf}{stronie}.

Zamiast tego omówimy krótko podstawowe zastosowanie pakietu
\texttt{siunitx}\footnote{Ten fragment wymaga przejrzenia.}, jest on
wystarczające do~prostego pisania wyrażeń typu $4\si{.m/s^{ 2 }}$.
Pełne jego omówienie znajduje~się na
\href{ftp://ftp.gust.org.pl/TeX/macros/latex/contrib/siunitx/siunitx.pdf}{stronie}.
Warto zwrócić uwagę, na~jedną z~możliwości jaką ten pakiet obiecuje:
łatwe tworzenia tabel, gdzie liczby~są wyrównane na~kropce
dziesiętnej. Jednak do~tej pory autorzy tego tekstu, nie~sprawdzili,
czy~ta opcja działa poprawnie.

\begin{enumerate}
\item Podstawową komendą pakietu jest \texttt{\tbs si\{...\}}. Przykładowo
  \begin{quote}
    \verb+\si{ kg }+ $\to \si{kg}$, \\
    \verb+\frac{ \si{ kg } }{ \si{ m^{ 3 } } }+
    $\to \frac{ \si{ kg } }{ \si{ m^{ 3 } } }$, \\
    \verb+\si{ m / s^{ 2 } }+ $\to \si{ m / s^{ 2 } }$.
  \end{quote}
\item Nazwy różnych jednostek fizycznych powinny być oddzielone
  spacją, np. powinno być ,,$\si{kg.m}$'', a~nie
  ,,$\textrm{kg}\textrm{m}$''. W~opcji \texttt{\tbs si{...}} można to
  osiągnąć wstawiając w~odpowiednim miejscu~,,.'', ilustruje to
  poniższy przykłady.
  \begin{quote}
    \verb+\si{ kg.m }+ $\to \si{kg.m}$.
  \end{quote}
...
\begin{enumerate}
\item Wyjaśniaj znaczenie wszystkich symboli i~zmiennych! Nie każdy
  musi wiedzieć, że~w~naszym sprawozdaniu $t$~to~czas, a~$L$~długość.
  Stałe matematyczne lub~fizyczne i~powszechnie znane funkcje nie
  muszą być definiowane jeśli są naprawdę powszechnie znane pod danym
  oznaczeniem. (Można z grubsza przyjąć podział, że~nie musimy
  definiować funkcji i~stałych wprowadzanych do poziomu szkoły
  średniej włącznie). W~razie wątpliwości lepiej napisz co~oznaczają
  --~raczej nikt na~ciebie za~to nie obrazi.
\item Każdy rysunek, wykres, tabela, itd., \tb{musi} być
  \tb{podpisany}.
\item Każdy wykres \tb{musi} mieć podpisane wszystkie
  \tb{osie}\footnote{W~przeciwnym razie może ciebie spotkać coś
    \href{https://xkcd.com/833/}{takiego}.}! Muszą też być podane
  \tb{jednostki} fizyczne wielkości przedstawionych na~wykresie.
...
\item Na końcu linii nie powinny~się znajdować ,,i'', ,,a'', ,,o'',
  ,,w'', ,,z'', ,,we'', ,,że''. Linia zaś nie powinna zaczynać~się
  od~słowa~,,się''. Warto wyrobić sobie nawyk pisania twardej spacji
  po tych wyrazach (patrz poprzedni punkt).

  Te wyrazy pozostawione w~takich niefortunnych miejscach nazywa się
  ,,sierotki''. Reguły te~wnikają z faktu, że~wyrazy takie jak
...
\item Należy rozróżniać łącznik (,,-''), półpauzę (,,--'') i~pauzę
  ...
  obiektu od~jego definicji. Przykładowo.

  ,,Pauza ---~znak typograficzny w~postaci poziomych kresek
  usytuowanych w pobliżu średniej linii pisma lub~nieco poniżej
...
\item \tb{Na~wszelki wypadek.} Aby~móc używać pakietu
  ...
  W~uproszczeniu, preambuła to część pliku pomiędzy od~jego początku
  (linii \texttt{\tbs documentclass[...]\{...\}}), do~początku
  właściwego tekstu, czyli~do~komendy \texttt{\tbs begin\{document\}}.
\item \LaTeX{} pozwala definiować użytkownikowi własne komendy. Aby~to
  zrobić należy wstawić do preambuły (tak jak w~poprzednim
  przykładzie) linie:
  \begin{quote}
    \verb+\newcommand{\nazwa_komendy}[ilość_zmiennych]{treść_komendy}+.
  \end{quote}
  Przykładowo:
  \begin{quote}
    \verb+\newcommand{\powt}[1]{$t^{ #1 }$}+,
  \end{quote}
  definiuje komendę która po~wpisaniu w~tekście ,,\verb+\powt{3}+''
  (nie trzeba, a wręcz nie powinno się wchodzić do trybu
  matematycznego) daje ,,\powt{3}''. Bardziej skomplikowany
  przykład~to:
  \begin{quote}
    \verb+\newcommand{\pd}[3]{ \frac{ \partial^{ #1 } { #2 } }+ \\
      \hspace{1em}\verb+{ \partial { #3 }^{ #1 } } }.+
  \end{quote}
  Wpisanie teraz w~trybie matematycznym\footnote{Proszę zauważyć,
    że~w~definicji komendy nie~użyliśmy znaku \$.}
  \verb+\pd{ 2 }{ y }{ x }+ daje $\pd{ 2 }{ y }{ x }$. Jeżeli jednak
  chcemy napisać $\pd{}{ x }{ y }$ nie~możemy pominąć pierwszego
  nawiasu wąsatego, lecz musimy pozostawić go pustym:
  \verb+\pd{ }{ y }{ x }+.

  \tb{Uwaga techniczna.} Nie wchodziliśmy na~tyle głęboko w~sposób
  ...
  \end{quote}
  to~\texttt{\tbs timestwo\{3\}} zostanie zastąpione przez \texttt{\{
    3 \} * 2}, następnie plik zostanie przetworzony (np.~do~PDFa)
  w~normalny sposób.
...
\item
  \href{http://artisan-studio.pl/popularne-bledy-typograficzne-czyli-jak-unikac-bykow}{\emph{Popularne
      błędy typograficzne,}}\emph{czyli jak unikać byków}. \\
  Adres:
  http://artisan-studio.pl/popularne-bledy-typograficzne-czyli-jak-unikac-bykow.
\item \emph{\TeX{} Users Group}, \href{http://tug.org/}{http://tug.org/}.
\end{enumerate}
